...
void ZDualCameraStereoSLS::onPatternsDecoded(std::vector<ZDecodedPatternPtr> decodedPatterns)
{
    ...
    Z3D::ZSimplePointCloudPtr cloud(nullptr);

    if (decodedPatterns.front()->fringePointsList().size()) {
        /// if available, use fringes
        ...
        cloud = triangulate(decodedPatterns[0]->intensityImg(),
